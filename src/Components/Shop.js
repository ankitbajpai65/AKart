    ...
    const populateSearch = (e) => {
        if (e.key === 'Enter') {
            ...
            if (filteredItems.length == 0)
                setNoFilteredItems(true)
        }
    }
    const populateSearchOnClick = (e) => {
        const filteredItems = itemToDisplay.filter((val) => val.title.toLowerCase().includes(input.toLowerCase()));
        setItemToDisplay(filteredItems);
        if (filteredItems.length == 0)
            setNoFilteredItems(true)
    }
                            ...
                            <ul className="row">
                                ...
                                <li className="offset-md-1 col-6 col-sm-5 col-5 col-md-3 mt-5 mt-md-0">
                                    <span className='position-relative'>
                                        <input type="text" placeholder='Search' className="searchBar" onChange={inputEvent} onKeyUp={populateSearch} />
                                        <SearchIcon className='searchIcon' onClick={populateSearchOnClick} ref={search} />
                                    </span>
                                </li>
                                ...
                                loading ?
                                    ...
                                    (
                                        (noFilteredItems == true) ?
                                            <h1 className="display-3 text-center mt-5 fw-semibold">Search Not Found!</h1> :
